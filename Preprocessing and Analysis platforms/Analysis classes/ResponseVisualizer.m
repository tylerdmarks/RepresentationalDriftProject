classdef ResponseVisualizer < Analyzer
    ...
    methods
        ...
        % function obj = plotPDGvNatMov_traces(obj, startcell)
        ...
        %     try cd('PDGvNatMov_traces')
        %     catch
        %         mkdir('PDGvNatMov_traces')
        %         cd('PDGvNatMov_traces')
        %     end
            
        %     gain = 100;

        %     PDG_RespMat = obj.RespData.PDG_RespMat;
        %     PDG_CCs = obj.StabilityData.PDG.PDG_CCs;
        %     NatMov_RespMat = obj.RespData.NatMov.NatMov_RespMat_onTime;
        %     NatMov_CCs = obj.StabilityData.NatMov.NatMov_CCs;
            
        %     [PDG_RespMat, NatMov_RespMat] = obj.normalizeALL(PDG_RespMat, NatMov_RespMat);

        %     disp('Press any key to move to next cell, click to save as png file');
        %     onFrames = size(NatMov_RespMat, 2);

        %     for gg = startcell:obj.numCells           
        %         if cell_vec(gg)
        %             f = figure;
        %             for s = 1:obj.num_sessions

        %                 subplot(obj.num_sessions, 5, (s*5-4):(s*5-3))
        %                 image(PDG_RespMat(:, :, gg, s)*gain);
        %                 title(['PDG neuron #' num2str(gg), ' D' num2str((s*7)-7) ';  between-trial CC: ' num2str(PDG_CCs(s, gg))]);
        %                 ylabel('Trials');
        %                 box off

        %                 subplot(obj.num_sessions, 5, (s*5-2):(s*5-1))
        %                 image(NatMov_RespMat(:, :, gg, s)*gain);
        %                 title(['NatMov neuron #' num2str(gg), ' D' num2str((s*7)-7) ';  between-trial CC: ' num2str(NatMov_CCs(s, gg))]);
        %                 ylabel('Trials');
        %                 box off
                        
        %                 if s > 1
        %                         subplot(obj.num_sessions, 5, s*5)
        %                         meanResp_1 = mean(NatMov_RespMat(:,:,gg, 1));
        %                         plot(meanResp_1,'linewidth',1.5,'color','c');
        %                         hold on
        %                         meanResp_2 = mean(NatMov_RespMat(:,:,gg, s));
        %                         plot(meanResp_2,'linewidth',1.5,'color','m');
        %                         hold off

        %                         SE_1 = std(NatMov_RespMat(:,:,gg,1))/sqrt(size(NatMov_RespMat,1));
        %                         SEM = patch([1:onFrames fliplr(1:onFrames)],[meanResp_1+SE_1 fliplr(meanResp_1-SE_1)],[0 0.8 1],'EdgeColor','none');
        %                         set(SEM,'FaceVertexAlphaData',.5,'facealpha','flat');
        %                         SE_2 = std(NatMov_RespMat(:,:,gg,s))/sqrt(size(NatMov_RespMat,1));
        %                         SEM = patch([1:onFrames fliplr(1:onFrames)],[meanResp_2+SE_2 fliplr(meanResp_2-SE_2)],[0.8 0 0.8],'EdgeColor','none');
        %                         set(SEM,'FaceVertexAlphaData',.5,'facealpha','flat');
        %                         xlim([1 onFrames])
        %                         xlabel('Frame');
        %                         ylabel('dF/F (norm)');
        ...
        %             end
        %             set(f,'color',[1 1 1])
        %             set(f,'Position',[20 20 1800 1000])
        %             w = waitforbuttonpress;
        %             if ~w
        %                 saveas(gcf, ['n' num2str(gg) '.png']);
        %                 uistack(f, 'top')
        %             end 
        %             close
        %         end
